...
def list_table_overview(db: Session) -> list[tuple[UUID, str, UUID | None, str | None]]:
    rows = physical_table_repo.list_tables_overview(db)
    return [(table.id, table.table_code, group_id, state) for table, group_id, state in rows]
