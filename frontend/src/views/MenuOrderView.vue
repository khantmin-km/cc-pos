...
// Cache add-ons per item to avoid expensive recomputation
const itemAddonsMap = computed(() => {
  const cache: { [key: string]: any[] } = {}
  displayItems.value.forEach(item => {
    cache[item.id] = menuStore.getAddonsForItem(item.id)
  })
  return cache
})
    ...
    <div v-if="!loading" class="menu-list">
      <div v-for="item in displayItems" :key="item.id" class="menu-item-row">
        ...
        <div class="menu-item-card">
          ...
          <button
            v-if="itemAddonsMap[item.id] && itemAddonsMap[item.id].length > 0"
            class="addons-toggle"
        ...
        <div v-if="expandedItem === item.id && itemAddonsMap[item.id] && itemAddonsMap[item.id].length > 0" class="addons-section">
          <div
            v-for="addon in itemAddonsMap[item.id]"
            :key="addon.id"
            class="addon-row"
