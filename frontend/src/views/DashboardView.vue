...
onMounted(async () => {
  try {
    ...
    // Load data - fetch open table groups to get order items
    await tableGroupsStore.fetchOpenGroups()

    // Order items are loaded per table group, not globally
    // For dashboard, we'll show data from open groups
    const groups = tableGroupsStore.backendGroups
    for (const group of groups) {
      try {
        const items = await ordersStore.orderItemsApi.getByTableGroup(group.id)
        ordersStore.orderItems.push(...items)
      } catch (e) {
        console.warn(`Failed to load order items for group ${group.id}:`, e)
      }
    }
