  ...
  async function fetchOrderItems(tableId: string) {
    ...
    try {
      ...
      if (table?.current_table_group_id) {
        ...
        const items = await orderItemsApi.getByTableGroup(table.current_table_group_id)
        orderItems.value = items
        console.log('[ORDERS] Fetched order items for table group:', table.current_table_group_id, items.length, 'items')
      } else {
